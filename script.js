...
function checkValue(qty) {
    ...
    if (input.value <= 0) {
        ...
        alert("Maximun quantity achieved");
        input.value = qty;
    }
}
...
function payNow(id) {
    ...
    r.onreadystatechange = function() {
        ...
        if (r.readyState == 4) {
            ...
            if (t == "1") {
            ...
            } else { // Payment completed. It can be a successful failure.
                ...
                var payment = {
                    "sandbox": true,
                    "merchant_id": "1221300", // Replace your Merchant ID
                    "return_url": "http://localhost/eShop/sigleProductView.php?id=" + id, // Important
                    "cancel_url": "http://localhost/eShop/sigleProductView.php?id=" + id, // Important
...
function buynow(price) {
    ...
    r.open("POST", "rbuynowprocess.php", true);
    r.send(f);
}
